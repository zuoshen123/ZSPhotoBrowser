...
- (void)iniaialUI
{
    ...
    _backGround.backgroundColor = XLPhotoBrowserBackgrounColor;
...
- (void)pan:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!_directionLock) {
            vertical = (fabs(translation.x) < fabs(translation.y)) && translation.y > 0;
            if (vertical) {
                [self changeBrowserState:BrowserStateBegin contentMode:UIViewContentModeScaleToFill isSingleTap:NO];
                currentZoomingScrollView.scrollview.scrollEnabled = NO;
            }
            _directionLock = YES;
        }
        if (vertical) {
            if (translation.y < 0) {
                currentZoomingScrollView.scrollview.scrollEnabled = YES;
            }
            [self translationForFactor:translation withLocation:touchPoint];
        }else{
            currentZoomingScrollView.scrollview.scrollEnabled = YES;
        }
    }else if (gesture.state == UIGestureRecognizerStateEnded){
        if (vertical) {
            if (translation.y >= self.xl_height*0.7) {
                [self changeBrowserState:BrowserStateDone contentMode:self.sourceImageView.contentMode isSingleTap:NO];
            }else{
                [self changeBrowserState:BrowserStateCancel contentMode:UIViewContentModeScaleToFill isSingleTap:NO];
            }
        }
...
- (void)showFirstImage
{
    ...
    self.sourceImageView.originalFrame = startRect;

    self.tempView.image = [self placeholderImageForIndex:self.currentImageIndex];
    self.tempView.frame = startRect;
    self.tempView.contentMode = self.sourceImageView.contentMode;
    ...
    self.scrollView.hidden = YES;
    [self.sourceImageView setIsWhiteState:YES];
    // 动画修改图片视图的frame , 居中同时放大
    [UIView animateWithDuration:XLPhotoBrowserShowImageAnimationDuration delay:0. usingSpringWithDamping:0.6 initialSpringVelocity:20 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.tempView.frame = targetRect;
        self.backGround.alpha = 1.0;
    } completion:^(BOOL finished) {
        [self.tempView removeFromSuperview];
        [self.sourceImageView setIsWhiteState:NO];
...
- (void)zoomingScrollView:(XLZoomingScrollView *)zoomingScrollView singleTapDetected:(UITapGestureRecognizer *)singleTap
{
    ...
    if (self.sourceImageView == nil) {
        if ([_delegate respondsToSelector:@selector(dismissPhotoBrowser:)]) {
    ...
    [self changeBrowserState:BrowserStateBegin contentMode:self.sourceImageView.contentMode isSingleTap:YES];
    [self changeBrowserState:BrowserStateDone contentMode:self.sourceImageView.contentMode isSingleTap:YES];
}

-(void)translationForFactor:(CGPoint) translation withLocation:(CGPoint)location{
    NSLog(@"*****%f******%f",translation.x,translation.y);
    CGFloat factor = translation.y / (self.xl_height*0.7);
    if (factor < 0) {
        factor = 0;
    }
    if (factor > 1.0) {
    ...
    self.backGround.alpha = 1.0-factor;
    CGFloat width = self.tempView.originalFrame.size.width * (1 - factor*2/3);
    CGFloat height = width * self.tempView.originalFrame.size.height / self.tempView.originalFrame.size.width;
    self.tempView.frame = CGRectMake(self.tempView.originalFrame.origin.x+translation.x, self.tempView.originalFrame.origin.y+translation.y, width ,height);
    self.tempView.center = [self getTempViewCenterPoint:location viewFrame:self.tempView.originalFrame withFactor:1.0-factor];
    if ([_delegate respondsToSelector:@selector(gestureCallbackPhotoBrowser:withGestureState:withFactor:)]) {
        [_delegate gestureCallbackPhotoBrowser:self withGestureState:BrowserStateChange withFactor:factor];
    }
}

-(CGPoint) getTempViewCenterPoint:(CGPoint)location viewFrame:(CGRect)frame withFactor:(CGFloat)factor{

    CGFloat tempX = (CGRectGetMidX(frame) - _startPoint.x);
    CGFloat offsetX = tempX*(1.0/3 + factor*2.0/3);
    CGFloat height = CGRectGetHeight(frame)*(1.0/3 + factor*2.0/3);
    CGFloat offsetY = (CGRectGetMinY(frame)-_startPoint.y) * factor + height/2;
    
//    CGFloat width1 = (CGRectGetMidX(frame)-_startPoint.x + CGRectGetWidth(frame)/2)*2;
//    CGFloat height1 = (CGRectGetMidY(frame)-_startPoint.y + CGRectGetHeight(frame)/2)*2;
//    CGFloat width2 = width1 * factor;
//    CGFloat height2 = height1 * factor;
//    CGFloat offsetX = (width2 - CGRectGetWidth(frame)*factor)/2;
//    CGFloat offsetY = (height2 - CGRectGetHeight(frame)*factor)/2;
    return CGPointMake(location.x + offsetX, location.y + offsetY);
}

-(void)changeBrowserState:(BrowserState)state contentMode:(UIViewContentMode)mode isSingleTap:(BOOL)singleTap{
    XLZoomingScrollView *currentZoomingScrollView = [self zoomingScrollViewAtIndex:self.currentImageIndex];
    if (state == BrowserStateBegin) {
        ...
        self.sourceImageView = [self sourceImageViewForIndex:self.currentImageIndex];
        self.sourceImageView.originalFrame = [self.sourceImageView.superview convertRect:self.sourceImageView.frame toView:self];;
        [self.sourceImageView setIsWhiteState:YES];
        self.tempView.contentMode = mode;
        self.tempView.clipsToBounds = YES;
        self.tempView.image = currentZoomingScrollView.currentImage;
        self.tempView.frame = CGRectMake( - currentZoomingScrollView.scrollview.contentOffset.x + currentZoomingScrollView.imageView.xl_x,  - currentZoomingScrollView.scrollview.contentOffset.y + currentZoomingScrollView.imageView.xl_y, currentZoomingScrollView.imageView.xl_width, currentZoomingScrollView.imageView.xl_height);
        [self addSubview:self.tempView];
        self.tempView.originalFrame = CGRectMake(self.tempView.xl_x, currentZoomingScrollView.imageView.xl_y, self.tempView.xl_width, self.tempView.xl_height);
    }else if (state == BrowserStateCancel){
        currentZoomingScrollView.scrollview.scrollEnabled = YES;
        [[UIApplication sharedApplication] setStatusBarHidden:YES withAnimation:UIStatusBarAnimationFade];
        _isAnimating = YES;
        [UIView animateWithDuration:XLPhotoBrowserShowImageAnimationDuration delay:0. usingSpringWithDamping:0.6 initialSpringVelocity:20 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            self.tempView.frame = self.tempView.originalFrame;
            self.backGround.alpha = 1.0;
        } completion:^(BOOL finished) {
            _isAnimating = NO;
            self.scrollView.hidden = NO;
        ...
        CGRect targetTemp = self.sourceImageView.originalFrame;
        _isAnimating = YES;
        [UIView animateWithDuration:0.2 animations:^{
            self.backGround.alpha = 0.0;
        } completion:^(BOOL finished) {
        }];
        [UIView animateWithDuration:XLPhotoBrowserHideImageAnimationDuration delay:0. options:UIViewAnimationOptionCurveEaseInOut animations:^{
    ...
    if ([_delegate respondsToSelector:@selector(gestureCallbackPhotoBrowser:withGestureState:withFactor:)] && !singleTap) {
        [_delegate gestureCallbackPhotoBrowser:self withGestureState:state withFactor:0.0];
    }
}
